...
function ac.player.__index:addSelect(u)
    if self == ac.player.self then jass.SelectUnit(u.handle, true) end
end

-- 命令玩家取消选择某单位
--	单位
function ac.player.__index:removeSelect(u)
    if self == ac.player.self then jass.SelectUnit(u.handle, false) end
end
...
for i = 1, 16 do
    jass.TriggerRegisterPlayerUnitEvent(j_trg, ac.player[i].handle,
                                        jass.EVENT_PLAYER_UNIT_SELECTED, nil)
    jass.TriggerRegisterPlayerUnitEvent(j_trg, ac.player[i].handle,
                                        jass.EVENT_PLAYER_UNIT_DESELECTED, nil)
end
