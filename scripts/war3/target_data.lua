...
base.target_filter = function(dest, target_data, is_skill)
    return function(self)
        if self:has_restriction '无敌' then return false end

        if is_skill and self:has_restriction '魔免' then return false end

        if not self:is_alive() then return false end

        if not self:is_visible(dest) then return false end

        if self:is_enemy(dest) then
            if not target_data:find '敌人' then return false end
        end
        ...
        if self:get_owner() == dest:get_owner() then
            if not target_data:find '玩家单位' then return false end
        end

        if self:is_ally(dest) then
            if not target_data:find '联盟' then return false end
        end

        if self:is_hero() then
            if target_data:find '非-英雄' then return false end
        else
            if target_data:find '英雄' then return false end
        end
